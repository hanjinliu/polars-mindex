...
@overload
def agg(g: GroupBy, funcs: str | Iterable[str]) -> DataFrame2Lv:
    ...

@overload
def agg(g: GroupBy, *aggs: str) -> DataFrame2Lv:
    ...
    
def agg(g: GroupBy, *aggs):
    ...
    index_df = g.df.select(g.by, *g.more_by).unique(maintain_order=True)
    main_df_cols = [c for c in g.df.columns if c not in index_df.columns]
    exprs: list[pl.Expr] = []
    level0 = []
    level1 = []
